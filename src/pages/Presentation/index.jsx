...
for (let i = 0; i < 4; i++) {
  data.push({
    ...
    name: `Slide ${i}`,
    owner: "me",
    modifiedDate: new Date().toLocaleDateString(undefined, {
      year: "numeric",
      month: "short",
      day: "numeric"
    }),
...
const TableOfPresentations = (props) => {
  ...
  const rowSelection = {
    onChange: (selectedRowKeys, selectedRows) => {
      ...
      if (selectedRowKeys.length == 0) {
        props.handleHasSelectedPresentation(false);
      } else {
        props.handleHasSelectedPresentation(true);
      }
    },
  ...
  return (
    <Table rowSelection={{ ...rowSelection }} dataSource={data}>
      ...
      <Column title="Modified" dataIndex="modifiedDate" key="modifiedDate" sorter={true} />
      <Column title="Created" dataIndex="createdDate" key="createdDate" sorter={true} />
      <Column
        title=""
        key="action"
        render={(_, record) => {
